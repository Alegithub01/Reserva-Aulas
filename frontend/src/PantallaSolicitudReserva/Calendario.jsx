...
const Calendario = () => {
  ...
  const fetchSchedule = async (aula) => {
    try {
      const response = await axios.get(`https://my-json-server.typicode.com/WilliamCallao/fake-api/solicitudes_individuales`);
      const reservas = response.data;
      const filteredReservas = reservas.filter(reserva => reserva.aulas.includes(aula));
      const schedule = filteredReservas.reduce((acc, reserva) => {
        if (!acc[reserva.fecha]) {
          acc[reserva.fecha] = [];
        }
        reserva.horario.forEach(hora => {
